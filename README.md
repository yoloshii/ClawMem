...
| `CLAWMEM_CONTRADICTION_INVALIDATE` | `false` | **v0.28.0.** A different subsystem from the two rows above — this one governs the **`decision-extractor` hook**, not the merge-time gate. Set to exactly `true` to let a contradiction that erodes a document to the `0.2` confidence floor also set `invalidated_at`, removing it from FTS *and* vector retrieval. Off by default: the hook logs `WOULD invalidate` and writes nothing. Confidence erosion runs either way. Eligible rows are `content_type='observation'` only. [Calibration guide](docs/guides/contradiction-invalidation.md). |
