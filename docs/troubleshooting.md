...
- **Under OpenClaw these lines are not visible**: the plugin surfaces hook stderr only on a non-zero exit, and a successful shadow run exits zero. **Arming is unsupported on that host** until shadow output is observable there — calibrating under Claude Code is not a substitute, because exposure depends on prompts, session traffic and candidate selection, all of which differ by host.
- If the proposals are frequent and look wrong, that is a classifier-precision signal (check `CLAWMEM_LLM_MODEL`), not a logging problem.
